...
(() => {
  ...
  style.textContent = `
    ...
    .wat-pop{position:absolute;z-index:2;color:#f5d76e;font:bold 1.25rem monospace;pointer-events:none;animation:wat-pop .45s ease-out forwards}@keyframes wat-pop{from{transform:translateY(0) scale(1);opacity:1}to{transform:translateY(-35px) scale(1.2);opacity:0}}
  ...
  for (let i = 0; i < MOLE_COUNT; i++) {
    ...
    hole.innerHTML = '<span class="wat-mole" aria-label="8-bit mole">●</span>';
    board.appendChild(hole);
  }

  let active = false, targetIndex = 0, hits = 0, timer = 0, popTimer = 0, seconds = 20;
  const updateHealth = () => {
  ...
  const pop = () => {
    ...
    popTimer = setTimeout(() => mole.classList.remove('up'), 650);
  };
  const end = won => {
    ...
    else panel.querySelector('#wat-message').textContent = 'Time up — try again!';
    songRows().forEach((row, i) => { if (destroyed.has(i)) row.hidden = true; });
  };
  const start = () => {
    targetIndex = currentSongIndex(); hits = 0; seconds = 20; active = true; panel.hidden = false;
    ...
    clearInterval(timer); timer = setInterval(() => { seconds--; updateHealth(); if (seconds <= 0) end(false); }, 1000);
  };
